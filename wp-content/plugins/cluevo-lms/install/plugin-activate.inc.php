...
$cluevo_plugin_db_version = '1.3.1';
...
function cluevo_create_database() {
  ...
  $sqlUsersToGroups = "CREATE TABLE $usersToGroupTable (
    ...
    date_added DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    date_modified TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id, group_id)) $charset_collate;";
  ...
  $sqlModules = "CREATE TABLE $moduleTable (
    ...
    UNIQUE INDEX module_name (module_name, lang_code)) $charset_collate;";
  dbDelta( $sqlModules );
  ...
  $sqlTreePerms = "CREATE TABLE $permTable (
    ...
    date_added TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (perm_id),
    UNIQUE INDEX item_perm (item_id, perm) ) $charset_collate;";
  ...
  $sqlDeps = "CREATE TABLE $depTable (
    ...
    date_added DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    date_modified TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (item_id, dep_id)) $charset_collate;";
  dbDelta ( $sqlDeps );
  ...
  $sqlModuleDeps = "CREATE TABLE $moduleDepTable (
    ...
    date_added DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    date_modified TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (module_id, dep_id)) $charset_collate;";
  ...
  $sqlCompetenceAreasTable = "CREATE TABLE $competenceAreasTable (
    ...
    UNIQUE KEY competence_area_name (competence_area_name)) $charset_collate;";
  ...
  $sqlModuleCompetencesTable = "CREATE TABLE $moduleCompetenceTable (
    ...
    UNIQUE INDEX competence_module (competence_id, module_id)) $charset_collate;";
  ...
  $sqlCompetencesToAreas = "CREATE TABLE $competencesToAreasTable (
    ...
    date_added DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (competence_id, competence_area_id)) $charset_collate;";
  ...
  $sqlCompetenceTable = "CREATE TABLE $competenceTable (
    ...
    UNIQUE KEY competence_name (competence_name)) $charset_collate;";
...
function cluevo_create_lms_pages() {
  ...
  if ( get_page_by_title( 'User', ARRAY_A, CLUEVO_PAGE_POST_TYPE) == NULL ) {
    $page = array(
      'post_title'  => 'User',
      'post_status' => 'publish',
  ...
  if ( get_page_by_title( 'Index', ARRAY_A, CLUEVO_PAGE_POST_TYPE) == NULL ) {
    $page = array(
      'post_title'  => 'Index',
      'post_status' => 'publish',
      'post_type'   => CLUEVO_PAGE_POST_TYPE,
  ...
  if ( get_page_by_title( 'Login', ARRAY_A, CLUEVO_PAGE_POST_TYPE) == NULL ) {
    $page = array(
      'post_title'  => 'Login',
      'post_status' => 'publish',
      'post_type'   => CLUEVO_PAGE_POST_TYPE,
  ...
  if ( get_page_by_title( 'Logout', ARRAY_A, CLUEVO_PAGE_POST_TYPE) == NULL ) {
    $page = array(
      'post_title'  => 'Logout',
      'post_status' => 'publish',
      'post_type'   => CLUEVO_PAGE_POST_TYPE,
...
function cluevo_create_cluevo_uploads_htaccess() {
  ...
  $content = "<FilesMatch \"\.(php|php3|phtml|php\.)$\">\n";
...
function cluevo_install_demos($treeId = 0) {
  $list = file_get_contents("https://www.cluevo.at/plugin-demo-modules/demos.txt");
  $list = explode("\n", $list);
  $count = 0;
  $messages = [];
  $errors = [];
  ...
  if (empty($treeId)) {
    $trees = cluevo_get_learning_structures($user);
    if (!empty($trees)) {
      $treeId = $trees[0]->item_id;
    }
  }

  $course = new CluevoItem();
  $course->parent_id = $treeId;
  $course->level = 1;
  $course->name = __("Demo Course", "cluevo");
  $courseId = cluevo_create_learning_structure_item($course);
  $course->metadata_id = cluevo_create_metadata_page($course, $trees[0]->metadata_id);
  cluevo_update_learning_structure_item($course);

  if ($courseId === false) {
    return false;
  }

  $chapter = new CluevoItem();
  $chapter->parent_id = $courseId;
  $chapter->name = __("Demo Chapter", "cluevo");
  $chapter->level = 2;
  $chapterId = cluevo_create_learning_structure_item($chapter);
  $chapter->metadata_id = cluevo_create_metadata_page($chapter, $course->metadata_id);
  cluevo_update_learning_structure_item($chapter);
  ...
  foreach ($list as $url) {
    if (!empty($url)) {
      ...
      $handled = false;
      $m = cluevo_handle_module_download($url, $errors, $messages, $handled);
      if (!empty($m)) {
        $module = new CluevoItem();
        $module->parent_id = $chapterId;
        $module->name = $m->module_name;
        $module->level = 3;
        $module->module_id = $m->module_id;
        cluevo_create_learning_structure_item($module);
        $module->metadata_id = cluevo_create_metadata_page($module, $chapter->metadata_id);
        cluevo_update_learning_structure_item($module);
      }
    }
  }
  return [ "installed" => $count, "messages" => $messages, "errors" => $errors ];
}
