...
function cluevo_get_tree_perms($strType, $intId, $intItemId = null) {
  global $wpdb;
  $cluevo_groups = $GLOBALS["cluevo_groups"];
  $treeTable = $wpdb->prefix . CLUEVO_DB_TABLE_TREE;
  $permTable = $wpdb->prefix . CLUEVO_DB_TABLE_TREE_PERMS;
  ...
  if (!empty($strType)) {
    ...
    if (!empty($intItemId)) {
      $sql .= " WHERE t.path LIKE CONCAT('%/', %d, '/%') OR t.item_id = %d";
      $args[] = $intItemId;
      $args[] = $intItemId;
    }
        ...
        foreach ($results as $key => $row) {
          ...
          if ($permType == "u") {
            ...
            if (empty($final[$id])) { // if a user has no specific user permission but has group permissions for an item
              ...
              foreach ($row as $prop => $value) { $perm->{$prop} = $value; }
              $perm->access_level = 0;
              $perm->perm_id = null;
              $perm->perm = "u:$intId";
              $perm->highest_group_access_level = json_decode(json_encode([ "perm" => $row->perm, "access_level" => $row->access_level, "group" => $row->group ]));
              $final[$id] = $perm;
            }
          }
        }
