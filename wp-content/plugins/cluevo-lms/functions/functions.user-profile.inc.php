...
function cluevo_get_level_from_exp($curExp) {
  ...
  $pct = round(($curExp / $next) * 100, 2);

  return ["current" => $userLevel, "next" => $next, "remaining" => $remaining, "exp" => $curExp, 'title' => $title, 'pct' =>  $pct];
}

function cluevo_get_users_competences($intUserId) {
  $comps = cluevo_get_competences();
  $scores = cluevo_get_users_competence_scores($intUserId);

  foreach ($comps as $compKey => $c) {
    $c->load_modules();
    $c->competence_score = 0;
    foreach ($scores as $scoreKey => $s) {
      if ($c->competence_id === $s->competence_id) {
        if (!empty($c->modules)) {
          foreach ($c->modules as $mKey => $m) {
            if (!empty($s->modules)) {
              foreach ($s->modules as $sm) {
                if ($m->module_id === $sm->id) {
                  $c->modules[$mKey]->competence_score = $sm->score;
                  $c->competence_score += $sm->score;
                }
              }
            }
          }
    ...
    $c->competence_score = ($c->competence_score > 1) ? 1 : $c->competence_score;
  }
...
1, SUM(s.score)) AS score, GROUP_CONCAT(CONCAT(s.module_id, ':', s.coverage, ':', s.score) SEPARATOR ';') AS modules
    FROM (
      ...
      LEFT JOIN $moduleProgressTable mp ON mc.module_id = mp.module_id AND mp.user_id = %d
  ) s
