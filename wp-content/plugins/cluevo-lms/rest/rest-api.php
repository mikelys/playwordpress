...
function cluevo_api_init_new_attempt(WP_REST_Request $request) {
  $userId = get_current_user_id();
  if ($userId > 0) {
    $item = cluevo_get_learning_structure_item($request["item_id"], $userId);
...
function cluevo_api_fetch_learning_structure_item($args) {
  $userId = get_current_user_id();
  return cluevo_get_learning_structure_item($args["item_id"], $userId);
}
    ...
    if (!empty($module)) {
      switch ($module->type_name) {
        ...
        case "video":
          ...
          break;
        default:
          do_action('cluevo_save_module_progress', [ "user_id" => $userId, "module" => $module, "request" => $request ]);
      }
    }
  } else {
    return "authentication required";
...
function cluevo_api_create_item(WP_REST_Request $request) {
  ...
  $level = 0;
  if ($parentId != 0) {
    $parent = cluevo_get_learning_structure_item($parentId, $userId);
    if (!empty($parent)) {
  ...
  return cluevo_create_learning_structure_item($tmpItem);
}
...
function cluevo_api_remove_item_module(WP_REST_Request $request) {
  ...
  $module = cluevo_get_module($moduleId);
  if (!empty($item)) {
    cluevo_remove_learning_structure_module_item($id);
  }
}

function cluevo_api_delete_item(WP_REST_Request $request) {
  $id = (int)$request['id'];
  return cluevo_remove_learning_structure_item($id);
}
